      ...
      <form className={`${submitted && "hidden"}`} action={endpoint} onSubmit={handleSubmit} method="POST">
        <div className="text-lg">
          ...
          <div className="lg:flex text-sm">
            <div className="pr-2">
              <h3>First Name</h3>
              <input type="text" className="form-control h-9 w-72 p-1 bg-white border-2 border-slate-300" value={firstName} onChange={(e) => setFirstName(e.target.value)} maxLength={256} required/><br/>
            </div>
            <div className="lg:pl-2">
              <h3>Last Name</h3>
              <input type="text" className="form-control h-9 w-72 p-1 bg-white border-2 border-slate-300" value={lastName} onChange={(e) => setLastName(e.target.value)} maxLength={256} required/><br/>
            </div>
          </div>
        </div>
        ...
        <input type="email" className="form-control h-9 w-72 p-1 bg-white border-2 border-slate-300" value={email} onChange={(e) => setEmail(e.target.value)} maxLength={256} required/><br/>
        <p className="text-lg mt-2">
          Subject <span className="text-sm">(required)</span>
        </p>
        <input type="text" className="form-control h-9 lg:w-96 w-80 p-1 bg-white border-2 border-slate-300" value={subject} onChange={(e) => setSubject(e.target.value)} maxLength={1024} required/><br/>        
        <p className="text-lg mt-2">
          Message <span className="text-sm">(required)</span>
        </p>
        <textarea rows={3} className="form-control lg:w-96 w-80 p-1 bg-white border-2 border-slate-300" value={message} onChange={(e) => setMessage(e.target.value)} maxLength={10*1024} required></textarea><br/>
